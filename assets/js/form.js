(function () {
  ...
  // entri "Obat-obatan Pribadi" dan "Perlengkapan Ibadah & Mandi" supaya
  // selalu tertaut ke jadwal resmi, bukan sekadar label generik.
  ...
  const CONDITION_PLAYBOOK = [
    { keywords: ['asma'], point: 'Riwayat asma terdeteksi — siapkan akses ke inhaler pribadi peserta dan hindari paparan debu/asap berlebih selama kegiatan.' },
    { keywords: ['jantung'], point: 'Riwayat gangguan jantung terdeteksi — hindari aktivitas fisik berat tanpa pengawasan dan siapkan rencana evakuasi medis terdekat.' },
    { keywords: ['diabetes', 'gula darah', 'gula'], point: 'Indikasi diabetes/gula darah terdeteksi — pastikan jadwal makan teratur dan siapkan camilan/gula darurat.' },
    { keywords: ['epilepsi', 'ayan', 'kejang'], point: 'Riwayat epilepsi/kejang terdeteksi — pastikan selalu ada pendamping yang tahu langkah pertolongan pertama saat kejang.' },
    { keywords: ['maag', 'lambung', 'gerd'], point: 'Riwayat maag/gangguan lambung terdeteksi — pastikan jadwal makan tidak terlambat dan sediakan obat lambung bila dibawa.' },
    { keywords: ['tekanan darah', 'hipertensi', 'darah tinggi', 'darah rendah'], point: 'Indikasi tekanan darah tidak stabil terdeteksi — pantau kondisi secara berkala terutama saat cuaca panas/aktivitas fisik.' },
    { keywords: ['sesak', 'napas'], point: 'Keluhan pernapasan terdeteksi — siapkan area istirahat dengan sirkulasi udara baik.' },
  ];
  ...
  function detectConditionPoints(text) {
    ...
    if (!points.length) {
      points.push(`Kondisi medis khusus dilaporkan ("${text.trim()}") — perlu ditinjau guru/staf medis untuk rencana penanganan yang sesuai.`);
    }
    return points;
  }
  ...
  function buildMedicalAttentionPlan(d) {
    ...
    if (d.has_drug_allergy === 'yes') {
      points.push(`Alergi obat: ${d.drug_allergy_detail || 'tidak dirinci'} — pastikan obat tersebut TIDAK diberikan; informasikan ke seluruh pendamping/panitia P3K.`);
      riskScore += 2;
    }
    if (d.has_food_allergy === 'yes') {
      points.push(`Alergi makanan: ${d.food_allergy_detail || 'tidak dirinci'} — koordinasikan dengan bagian konsumsi agar menu tidak mengandung bahan tersebut.`);
      riskScore += 2;
    }
    if (d.has_other_allergy === 'yes') {
      points.push(`Alergi lain: ${d.other_allergy_detail || 'tidak dirinci'} — siapkan langkah antisipasi (mis. jauhkan dari pemicu, sediakan obat alergi).`);
      riskScore += 1;
    }
    if (Array.isArray(d.medications) && d.medications.length) {
      points.push(`Membawa ${d.medications.length} jenis obat pribadi — pastikan jadwal minum obat (${d.medications.map((m) => m.schedule).filter(Boolean).join(', ') || 'sesuai catatan peserta'}) tetap terpantau.`);
    }
    if (d.has_personal_issue === 'yes') {
      if (d.wants_to_explain === 'yes' && d.issue_detail) {
        points.push(`Peserta menandai ada masalah pribadi dan bersedia bercerita — perlu tindak lanjut personal & rahasia dari ${d.target_staff_name || 'guru/staf yang dipilih'} sesegera mungkin.`);
        riskScore += 2;
      } else {
        points.push(`Peserta menandai ada masalah pribadi namun belum ingin bercerita — cukup pastikan ${d.target_staff_name || 'guru/staf yang dipilih'} tetap terbuka/siap dihubungi peserta kapan saja.`);
        riskScore += 1;
      }
    }

    if (!points.length) {
      points.push('Tidak ada kondisi medis, alergi, atau masalah pribadi yang dilaporkan — penanganan standar/umum berlaku.');
    }
  ...
  function buildReview() {
    ...
    html += `<div class="review-row"><span class="review-row__label">Rencana</span><span class="review-row__value">${d.medical_attention_plan.points.map(escapeHtml).join('<br/>')}</span></div>`;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      planNote.textContent = `Rencana penanganan medis otomatis tersusun (prioritas: ${data.medical_attention_plan.priority}). Guru/staf medis dapat melihat rinciannya di panel admin.`;
    } catch (err) {
      console.error(err);
