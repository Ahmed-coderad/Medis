(function () {
  ...
  function updateTeacherNote() {
    ...
    const step5Name = document.getElementById('step5TeacherName');
    const step5NameInline = document.getElementById('step5TeacherNameInline');
    if (selectedStaff) {
      nameLabel.textContent = selectedStaff.name;
      if (step5Name) step5Name.textContent = selectedStaff.name;
      if (step5NameInline) step5NameInline.textContent = selectedStaff.name;
  ...
  // Mengunci pilihan Guru/Staf Medis saat Ubah Data: dipakai HANYA dari data
  // yang sudah tersimpan di dokumen peserta sendiri (target_staff_id/name/
  // whatsapp) — TIDAK bergantung pada `staffList` (yang dimuat async dan
  // hanya berisi staf yang masih aktif). Ini sengaja dibuat begini karena
  // dua alasan: (1) andai daftar staf belum selesai dimuat atau staf yang
  // dulu dipilih sudah dinonaktifkan admin, "Kirim Perubahan" tetap harus
  // berhasil tanpa peserta diharuskan memilih ulang; (2) guru/staf yang
  // sudah dipilih di pengiriman pertama memang tidak boleh diganti lewat
  // pengubahan data selanjutnya.
  function lockTargetStaffField(d) {
    selectedStaff = {
      id: d.target_staff_id,
      name: d.target_staff_name || '-',
      whatsapp: d.target_staff_whatsapp || '',
    };
    const selectField = document.getElementById('targetStaffField');
    const lockedField = document.getElementById('targetStaffLockedField');
    const sel = document.getElementById('target_staff');
    if (selectField) selectField.hidden = true;
    if (sel) sel.required = false;
    if (lockedField) {
      lockedField.hidden = false;
      document.getElementById('targetStaffLockedName').textContent = selectedStaff.name;
    }
    updateTeacherNote();
  }
  ...
  let supplySettingsUnsub = null;
  function loadSupplySettings() {
    return new Promise((resolve) => {
      let settled = false;
      supplySettingsUnsub = db.collection('settings').doc('supplyWindow').onSnapshot(
        (doc) => {
          supplySettings = doc.exists ? doc.data() : null;
          if (!settled) { settled = true; resolve(); }
        },
        (err) => {
          console.error('Gagal memuat jadwal perlengkapan:', err);
          supplySettings = null;
          if (!settled) { settled = true; resolve(); }
        },
      );
  ...
  function wireYesNoReveal(radioName, revealId, showOnValue = 'yes') {
    ...
    radios.forEach((r) => {
      r.addEventListener('change', () => {
        ...
        // Bip perhatian singkat saat peserta menandai "Ya" pada pertanyaan
        // alergi — dipasangkan dengan pulse-dot merah pada .alert-box__title.
        if (r.checked && r.value === showOnValue && window.SoundFX) window.SoundFX.alert();
      });
    });
  }
  ...
  document.querySelectorAll('input[name="has_personal_issue"]').forEach((r) => {
    r.addEventListener('change', () => {
      ...
      if (r.value === 'yes' && window.SoundFX) window.SoundFX.alert();
      if (r.value !== 'yes') {
  ...
  // guru/staf medis. Panel admin menampilkan hasilnya di detail peserta dan
  // di tab "Analisis Medis". Mesin aturannya sendiri ada di
  // assets/js/medical-plan.js supaya dipakai bersama oleh formulir ini dan
  // panel admin (satu sumber kebenaran, tidak pernah berbeda di dua tempat).
  // ==========================================================================
  function buildMedicalAttentionPlan(d) {
    return window.MedicalPlan.build(d);
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // issue_detail (isi "Ceritakan secara singkat" di Step 5) SENGAJA
      // dikeluarkan dari data yang disimpan ke /participants — lihat
      // wellbeing_notes di bawah untuk alasannya.
      const { issue_detail, ...participantData } = data;
      ...
      if (editingRecord && editingRecord.id) {
        ...
        participantRef = db.collection('participants').doc(docId);
        // Dipakai .set(..., {merge:true}) alih-alih .update() dengan sengaja:
        // .update() GAGAL dengan error "No document to update" jika dokumen
        // ini sudah dihapus admin (mis. dibersihkan setelah acara selesai)
        // sementara localStorage di perangkat peserta masih menyimpan id
        // lama itu. .set(..., {merge:true}) aman untuk DUA kondisi sekaligus:
        // jika dokumen masih ada, perilakunya sama seperti update (field lain
        // yang tidak dikirim tetap utuh); jika dokumen sudah tidak ada,
        // Firestore akan membuatnya kembali dari data ini (dievaluasi lewat
        // aturan "allow create" di firestore.rules) alih-alih menolak
        // permintaan — sehingga peserta tidak pernah terkunci hanya karena
        // data lamanya sudah dibersihkan admin.
        // submitted_at diisi ulang dari cache lokal (waktu pengiriman
        // PERTAMA) supaya tanggal itu tetap benar pada kedua kondisi di atas.
        batch.set(participantRef, {
          ...participantData,
          submitted_at: firebase.firestore.Timestamp.fromMillis(editingRecord.submitted_at || Date.now()),
          updated_at: firebase.firestore.FieldValue.serverTimestamp(),
          revision_count: (editingRecord.revision_count || 0) + 1,
        }, { merge: true });
      } else {
        // Dibuat lewat .doc() (bukan .add()) supaya ID dokumennya sudah
        // diketahui SEBELUM dikirim — dibutuhkan untuk menautkan dokumen
        // /wellbeing_notes dengan ID yang sama pada batch yang sama.
        participantRef = db.collection('participants').doc();
        docId = participantRef.id;
        batch.set(participantRef, {
          ...participantData,
      ...
      // Catatan kesejahteraan (Step 5): disimpan di koleksi TERPISAH yang
      // hanya bisa dibaca admin utama ATAU akun admin yang staff_id-nya
      // cocok dengan guru/staf yang dipilih peserta (lihat firestore.rules,
      // match /wellbeing_notes/{participantId}). Selalu disinkronkan pada
      // setiap pengiriman (termasuk dikosongkan bila peserta tidak lagi
      // ingin bercerita) supaya tidak ada cerita lama yang tertinggal.
      const wellbeingRef = db.collection('wellbeing_notes').doc(docId);
      batch.set(wellbeingRef, {
        full_name: data.full_name,
        class: data.class,
        target_staff_id: data.target_staff_id,
        target_staff_name: data.target_staff_name,
        issue_detail: (data.has_personal_issue === 'yes' && data.wants_to_explain === 'yes') ? issue_detail : '',
  ...
  function submitSuppliesFollowup(localRec) {
    ...
    const scheduleLabel = currentScheduleLabel();
    const d = localRec.full_data || {};
    const payload = {
      // Field identitas inti disertakan (bukan hanya data perlengkapan) agar
      // permintaan ini tetap valid meskipun ternyata dokumennya sudah
      // dihapus admin — lihat penjelasan lengkap di submit handler utama
      // mengenai kenapa .set(merge:true) dipakai alih-alih .update().
      full_name: d.full_name, class: d.class, major: d.major, role: d.role,
      guardian_name: d.guardian_name, guardian_phone: d.guardian_phone,
      submitted_at: firebase.firestore.Timestamp.fromMillis(localRec.submitted_at || Date.now()),
    ...
    db.collection('participants').doc(localRec.id).set(payload, { merge: true })
      .then(() => {
        localRec.supplies_done = true;
        saveLocalRecord(localRec);
        activeReminderLocalRec = null;
        document.getElementById('suppliesFollowup').hidden = true;
  ...
  function showReminder(localRec, bucket) {
    ...
    const scheduleLabel = currentScheduleLabel();
    const scheduleSuffix = scheduleLabel ? ` Jadwal resmi: ${scheduleLabel}.` : '';
    title.textContent = 'Alarm Pengingat Perlengkapan Obat, Ibadah & Mandi';
    text.textContent = `Pengingat pukul ${bucket} — lengkapi perlengkapan obat, ibadah & mandi kamu, ${localRec.full_name}.${scheduleSuffix}`;

    // PENTING: jangan ambil nomor HP dari `form.guardian_phone.value` — pada
    // kunjungan susulan (skenario UTAMA reminder ini muncul), formulir belum
    // pernah diisi sesi ini sehingga nilainya selalu kosong dan tombol
    // WhatsApp jadi mati (href="#"). Ambil dari data yang tersimpan lokal
    // saat pengiriman pertama, dengan urutan fallback yang masuk akal.
    const fd = (localRec && localRec.full_data) || {};
    const recipientPhone = fd.guardian_phone || fd.guardian_alt_phone || fd.participant_phone
      || (form.guardian_phone ? form.guardian_phone.value : '');
    const msg = `Assalamu'alaikum, mengingatkan untuk melengkapi perlengkapan obat, ibadah & mandi ananda ${localRec.full_name} untuk kegiatan sekolah.${scheduleSuffix} Terima kasih.`;
    const waLink = toWaLink(recipientPhone, msg);
    if (waLink) {
      waBtn.href = waLink;
      waBtn.classList.remove('is-disabled');
      waBtn.removeAttribute('aria-disabled');
    } else {
      // Tidak ada nomor HP yang bisa dituju sama sekali (kasus sangat
      // jarang) — nonaktifkan tombol dengan jelas alih-alih menyisakan
      // link mati (href="#") yang terlihat seperti tombol tidak berfungsi.
      waBtn.href = '#';
  ...
  // Dipakai oleh pemeriksa visibilitychange di bawah supaya pengingat segera
  // diperiksa ulang saat tab yang sempat berada di latar belakang (umum di
  // ponsel, di mana setInterval bisa ditunda browser) dibuka kembali.
  let activeReminderLocalRec = null;
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && activeReminderLocalRec) {
      checkReminders(activeReminderLocalRec);
    }
  });
  ...
  Promise.all([loadStaff(), loadSupplySettings()]).then(() => {
    ...
    if (localRec && !localRec.supplies_done) {
      ...
      if (phase === 'open') {
        ...
        activeReminderLocalRec = localRec;
