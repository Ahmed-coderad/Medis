...
(function () {
  ...
  function renderStats() {
    ...
    const osis = allRecords.filter((r) => r.role === 'Pengurus OSIS').length;
    const umum = allRecords.filter((r) => r.role === 'Peserta Umum').length;
    const panitia = allRecords.filter((r) => r.role === 'Panitia').length;
    ...
    document.getElementById('statsGrid').innerHTML = `
      ...
      <div class="stat-card">
        <div class="stat-card__value">${osis}</div>
        <div class="stat-card__label">Pengurus OSIS</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${umum}</div>
        <div class="stat-card__label">Peserta Umum</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${panitia}</div>
  ...
  const suppliesFilter = document.getElementById('suppliesFilter');
  [searchInput, roleFilter, suppliesFilter].forEach((el) => el.addEventListener('input', renderTable));

  function getFiltered() {
    const q = searchInput.value.trim().toLowerCase();
    const role = roleFilter.value;
    const supplies = suppliesFilter.value;
    return allRecords.filter((r) => {
      if (q && !`${r.full_name} ${r.class} ${r.major}`.toLowerCase().includes(q)) return false;
      if (role && r.role !== role) return false;
      if (supplies === 'done' && r.supplies_skipped) return false;
      if (supplies === 'pending' && !r.supplies_skipped) return false;
      return true;
    });
  }
  ...
  function suppliesStatusBadge(r) {
    if (r.supplies_skipped) return '<span class="badge badge--warn">Belum lengkap</span>';
  ...
  function renderTable() {
    ...
    recordsBody.innerHTML = records.map((r) => {
      return `
        <tr>
          ...
          <td>${suppliesStatusBadge(r)}</td>
          <td>
            <div class="row-actions">
              <button class="link-btn" data-detail="${r.id}">Lihat detail</button>
              <button class="link-btn" data-pdf="${r.id}">Unduh PDF</button>
              <button class="link-btn" data-contact="${r.id}">Hubungi</button>
              <button class="link-btn link-btn--danger" data-delete="${r.id}">Hapus data</button>
            </div>
          </td>
        </tr>
      `;
    ...
    recordsBody.querySelectorAll('[data-detail]').forEach((btn) => {
      btn.addEventListener('click', () => openDetail(btn.dataset.detail));
    });
    recordsBody.querySelectorAll('[data-pdf]').forEach((btn) => {
      btn.addEventListener('click', () => {
    ...
    recordsBody.querySelectorAll('[data-delete]').forEach((btn) => {
  ...
  function downloadPdfForRecord(r) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ unit: 'pt', format: 'a4' });
    renderParticipant(doc, r);
    doc.save(buildPdfFilename(r.full_name));
  }

  pdfBtn.addEventListener('click', () => {
    if (!currentRecord) return;
    downloadPdfForRecord(currentRecord);
  });
  ...
  // ==========================================================================
  // TAB "LAPORAN MEDIS ACARA" — mencatat kondisi kesehatan yang muncul
  // SELAMA acara berlangsung (mis. pusing, jatuh, demam mendadak), berbeda
  // dari data kesehatan yang peserta isi sendiri SEBELUM acara (yang
  // ditampilkan di tab Analisis Medis). Boleh ditautkan ke peserta terdaftar
  // ATAU dicatat manual (mis. untuk guru pendamping/tamu yang bukan peserta
  // formulir). Disimpan di koleksi Firestore terpisah `medical_logs` supaya
  // bisa diunduh sebagai laporan medis acara (CSV/PDF) setelah kegiatan
  // selesai.
  // ==========================================================================
  (function () {
    const form = document.getElementById('eventLogForm');
    if (!form) return; // panel tidak ada di halaman ini

    const participantSelect = document.getElementById('logParticipantSelect');
    const personNameInput = document.getElementById('logPersonName');
    const personClassInput = document.getElementById('logPersonClass');
    ...
    const severityFilter = document.getElementById('logSeverityFilter');
    const statsGrid = document.getElementById('medicalLogStatsGrid');
    const tbody = document.getElementById('medicalLogBody');
    const emptyState = document.getElementById('medicalLogEmptyState');

    let logs = [];
    let unsubLogs = null;
    let lastKnownParticipantIds = '';

    // Isi ulang daftar peserta terdaftar setiap kali data peserta berubah,
    // supaya guru/staf bisa memilih langsung tanpa mengetik ulang nama/kelas.
    function refreshParticipantOptions() {
      const idsKey = allRecords.map((r) => r.id).join(',');
      if (idsKey === lastKnownParticipantIds) return;
      lastKnownParticipantIds = idsKey;
      const current = participantSelect.value;
      const sorted = allRecords.slice().sort((a, b) => (a.full_name || '').localeCompare(b.full_name || ''));
      participantSelect.innerHTML = '<option value="">— Pilih dari peserta terdaftar —</option>'
        + sorted.map((r) => `<option value="${escapeHtml(r.id)}">${escapeHtml(r.full_name)} (${escapeHtml(r.class)})</option>`).join('');
      if (sorted.some((r) => r.id === current)) participantSelect.value = current;
    }
    const origRenderStats = renderStats;
    renderStats = function patchedRenderStats() { // eslint-disable-line no-func-assign
      origRenderStats();
    ...
    participantSelect.addEventListener('change', () => {
      ...
      const r = allRecords.find((x) => x.id === participantSelect.value);
      if (r) {
        personNameInput.value = '';
        personNameInput.placeholder = r.full_name;
        personClassInput.value = r.class || '';
      }
    });

    function severityBadgeClass(sev) {
      if (sev === 'Berat/Darurat') return 'badge--alert';
    ...
    function renderLogStats() {
      ...
      const ringan = logs.filter((l) => l.severity === 'Ringan').length;
      const sedang = logs.filter((l) => l.severity === 'Sedang').length;
      const berat = logs.filter((l) => l.severity === 'Berat/Darurat').length;
      statsGrid.innerHTML = `
        <div class="stat-card"><div class="stat-card__value">${total}</div><div class="stat-card__label">Total catatan</div></div>
        <div class="stat-card"><div class="stat-card__value">${ringan}</div><div class="stat-card__label">Ringan</div></div>
        <div class="stat-card"><div class="stat-card__value">${sedang}</div><div class="stat-card__label">Sedang</div></div>
        <div class="stat-card stat-card--alert"><div class="stat-card__value">${berat}</div><div class="stat-card__label">Berat / Darurat</div></div>
      `;
    }
    ...
    function getFilteredLogs() {
      ...
      return logs.filter((l) => {
        if (q && !`${l.person_name} ${l.person_class}`.toLowerCase().includes(q)) return false;
    ...
    function renderLogTable() {
      ...
      tbody.innerHTML = filtered.map((l) => {
        const when = l.recorded_at && l.recorded_at.toDate ? l.recorded_at.toDate().toLocaleString('id-ID') : '—';
        return `
          <tr>
            <td>${escapeHtml(when)}</td>
            <td>${escapeHtml(l.person_name)}</td>
            <td>${escapeHtml(l.person_class || '-')}</td>
            <td>${escapeHtml(l.condition)}</td>
            <td>${escapeHtml(l.action_taken || '-')}</td>
            <td><span class="badge ${severityBadgeClass(l.severity)}">${escapeHtml(l.severity)}</span></td>
            <td>${escapeHtml(l.recorded_by || '-')}</td>
            <td><button class="link-btn link-btn--danger" data-log-delete="${l.id}">Hapus</button></td>
          </tr>
        `;
      }).join('');
      tbody.querySelectorAll('[data-log-delete]').forEach((btn) => {
        btn.addEventListener('click', async () => {
          if (!confirm('Hapus catatan kondisi kesehatan ini secara permanen?')) return;
          try {
            await db.collection('medical_logs').doc(btn.dataset.logDelete).delete();
            if (window.SoundFX) window.SoundFX.click();
          } catch (err) {
            alert('Gagal menghapus catatan: ' + (err && err.message ? err.message : err));
          }
        });
      });
    }

    [searchInput, severityFilter].forEach((el) => el.addEventListener('input', renderLogTable));

    function subscribeToLogs() {
      unsubLogs = db.collection('medical_logs').orderBy('recorded_at', 'desc')
        .onSnapshot((snap) => {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const linkedParticipant = allRecords.find((x) => x.id === participantSelect.value) || null;
      const personName = linkedParticipant ? linkedParticipant.full_name : personNameInput.value.trim();
      const personClass = linkedParticipant ? linkedParticipant.class : personClassInput.value.trim();
      const condition = conditionInput.value.trim();

      if (!personName) { errEl.textContent = 'Pilih peserta terdaftar atau ketik nama manual.'; errEl.hidden = false; return; }
      if (!condition) { errEl.textContent = 'Isi kondisi/gejala yang dialami.'; errEl.hidden = false; return; }

      submitBtn.disabled = true;
      submitBtn.textContent = 'Menyimpan...';
      try {
        await db.collection('medical_logs').add({
          participant_id: linkedParticipant ? linkedParticipant.id : null,
          person_name: personName,
          person_class: personClass || '',
          severity: severitySelect.value,
          condition,
          action_taken: actionInput.value.trim(),
          recorded_by: (auth.currentUser && auth.currentUser.email) || 'Admin',
          recorded_at: firebase.firestore.FieldValue.serverTimestamp(),
        });
        form.reset();
        personNameInput.placeholder = 'Nama (bila bukan peserta terdaftar)';
        if (window.SoundFX) window.SoundFX.success();
      } catch (err) {
        errEl.textContent = 'Gagal menyimpan catatan: ' + (err && err.message ? err.message : err);
        errEl.hidden = false;
      } finally {
    ...
    document.getElementById('logExportCsvBtn').addEventListener('click', () => {
      ...
      const headers = ['Waktu', 'Nama', 'Kelas', 'Tingkat', 'Kondisi/Gejala', 'Tindakan', 'Dicatat oleh'];
      const escapeCsv = (v) => `"${String(v === undefined || v === null ? '' : v).replace(/"/g, '""')}"`;
      const lines = [headers.join(',')];
      rows.forEach((l) => {
        const when = l.recorded_at && l.recorded_at.toDate ? l.recorded_at.toDate().toLocaleString('id-ID') : '';
        lines.push([when, l.person_name, l.person_class, l.severity, l.condition, l.action_taken, l.recorded_by].map(escapeCsv).join(','));
      });
      const blob = new Blob(['\uFEFF' + lines.join('\n')], { type: 'text/csv;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'laporan-medis-acara.csv';
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    ...
    // ---------- Ekspor PDF (ringkasan laporan medis acara) ----------
    document.getElementById('logExportPdfBtn').addEventListener('click', () => {
      const rows = getFilteredLogs();
      const { jsPDF } = window.jspdf;
      const doc = new jsPDF({ unit: 'pt', format: 'a4' });
      drawHeader(doc, 'Laporan Medis Acara');
      let y = 130;
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9.5);
      doc.setTextColor(...MUTED);
      doc.text(`Dicetak: ${new Date().toLocaleString('id-ID')} · ${rows.length} catatan`, MARGIN, y);
      y += 20;

      if (!rows.length) {
        doc.setTextColor(...TEXT);
        doc.setFontSize(11);
        doc.text('Belum ada catatan kondisi kesehatan selama acara.', MARGIN, y + 10);
      } else {
        rows.forEach((l, idx) => {
          ...
          doc.setTextColor(...BRAND_DARK);
          doc.text(`${l.person_name}${l.person_class ? ` (${l.person_class})` : ''}`, MARGIN, y);
          doc.setFont('helvetica', 'normal');
          doc.setFontSize(9);
          doc.setTextColor(l.severity === 'Berat/Darurat' ? ALERT[0] : MUTED[0], l.severity === 'Berat/Darurat' ? ALERT[1] : MUTED[1], l.severity === 'Berat/Darurat' ? ALERT[2] : MUTED[2]);
          doc.text(`${when} · Tingkat: ${l.severity}`, MARGIN, y + 13);
          y += 28;
          y = field(doc, y, 'Kondisi/gejala', l.condition || '-');
          y = field(doc, y, 'Tindakan', l.action_taken || '-');
          y = field(doc, y, 'Dicatat oleh', l.recorded_by || '-');
          y += 6;
        });
      }
      doc.save('laporan-medis-acara.pdf');
    });
  })();
})();
