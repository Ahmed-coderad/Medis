// ============================================================================
// AKUN ADMIN — util bersama (dipakai oleh proses masuk & tab "Akun Admin").
// Akun tambahan (dibuat lewat tab "Akun Admin") tidak memakai email asli —
// mereka masuk pakai "User ID" pilihan sendiri. Di balik layar, Firebase
// Authentication tetap butuh format email, jadi setiap User ID dipetakan ke
// satu email sintetis internal (tidak pernah ditampilkan sebagai "email").
// ============================================================================
const ADMIN_LOGIN_DOMAIN = 'admin.medcheck.local';

// Akun utama/bawaan sistem (satu-satunya yang boleh masuk lewat Email, dan
// satu-satunya yang fitur "Ubah Password"-nya sengaja disembunyikan total
// di tab "Akun Admin" — email & passwordnya tidak boleh diubah/dihapus/
// diedit lewat portal ini sama sekali).
const PRIMARY_ADMIN_EMAIL = 'rizkyahmeddarmawan98@gmail.com';
...
function userIdToInternalEmail(userId) {
  return `${userId}@${ADMIN_LOGIN_DOMAIN}`;
}
...
(function () {
  ...
  const modeEmailBtn = document.getElementById('loginModeEmailBtn');
  const modeUserIdBtn = document.getElementById('loginModeUserIdBtn');

  let allRecords = []; // cache of everything the admin is allowed to read
  let unsubscribeSnapshot = null;
  let loginMode = 'email'; // 'email' (akun utama) atau 'userid' (akun tambahan)

  // ---------- Placeholder animasi mengetik pada field Email/User ID ----------
  let idTypewriterTimer = null;
  let currentIdentifierPhrase = '';
  function stopIdentifierTypewriter() {
    if (idTypewriterTimer) { clearTimeout(idTypewriterTimer); idTypewriterTimer = null; }
  }
  function startIdentifierTypewriter(phrase) {
    stopIdentifierTypewriter();
    currentIdentifierPhrase = phrase;
    let charIndex = 0;
    let deleting = false;
    const typeSpeed = 55;
    const holdMs = 1600;
    ...
    function tick() {
      if (identifierInput.value) { identifierInput.placeholder = phrase; return; } // pengguna sedang mengetik
      if (!deleting) {
        charIndex += 1;
        identifierInput.placeholder = phrase.slice(0, charIndex);
        if (charIndex >= phrase.length) {
          deleting = true;
          idTypewriterTimer = setTimeout(tick, holdMs);
          return;
        }
        idTypewriterTimer = setTimeout(tick, typeSpeed);
      } else {
        charIndex -= 1;
        identifierInput.placeholder = phrase.slice(0, charIndex);
        if (charIndex <= 0) {
          deleting = false;
          idTypewriterTimer = setTimeout(tick, 400);
          return;
        }
        idTypewriterTimer = setTimeout(tick, deleteSpeed);
      }
    }
    tick();
  }
  identifierInput.addEventListener('focus', () => {
    if (!identifierInput.value) { stopIdentifierTypewriter(); identifierInput.placeholder = currentIdentifierPhrase; }
  });
  identifierInput.addEventListener('blur', () => {
    if (!identifierInput.value) startIdentifierTypewriter(currentIdentifierPhrase);
  });

  // ---------- Toggle mode masuk: Email (akun utama) / User ID (akun tambahan) ----------
  function setLoginMode(mode) {
    loginMode = mode;
    modeEmailBtn.classList.toggle('is-active', mode === 'email');
    modeUserIdBtn.classList.toggle('is-active', mode === 'userid');
    identifierInput.value = '';
    loginError.hidden = true;
    if (mode === 'email') {
      identifierLabel.textContent = 'Email admin';
      startIdentifierTypewriter(PRIMARY_ADMIN_EMAIL);
    } else {
      identifierLabel.textContent = 'User ID admin';
      startIdentifierTypewriter('RizkyAhmedDarmawan');
    }
  }
  modeEmailBtn.addEventListener('click', () => { setLoginMode('email'); if (window.SoundFX) window.SoundFX.click(); });
  modeUserIdBtn.addEventListener('click', () => { setLoginMode('userid'); if (window.SoundFX) window.SoundFX.click(); });
  setLoginMode('email'); // mulai animasi placeholder sejak halaman dimuat
  ...
  auth.onAuthStateChanged(async (user) => {
    if (user) {
      ...
      // Akun tambahan pakai email SINTETIS internal yang tidak bermakna bagi
      // pengguna — tampilkan User ID & labelnya (bila ada) sebagai gantinya.
      try {
        const doc = await db.collection('admin_accounts').doc(user.uid).get();
        if (doc.exists) {
          const d = doc.data();
          nameEl.textContent = d.label ? `${d.label} (${d.user_id})` : (d.user_id || user.email || '');
        }
      } catch (e) { /* biarkan tampil email/kosong bila gagal memeriksa */ }
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      if (loginMode === 'email') {
        ...
        // Akun tambahan TIDAK boleh masuk lewat Email — hanya akun
        // utama/bawaan sistem yang boleh. Baru bisa dipastikan setelah
        // berhasil masuk (untuk tahu UID-nya); kalau ternyata akun
        // tambahan, langsung keluarkan lagi & tolak.
        const doc = await db.collection('admin_accounts').doc(cred.user.uid).get();
        if (doc.exists) {
          await auth.signOut();
          throw { code: 'custom/email-not-allowed' };
        }
      } else {
        const userId = sanitizeUserId(identifier);
        if (!userId) throw { code: 'custom/userid-required' };
        const idxDoc = await db.collection('admin_login_index').doc(userId).get();
        if (!idxDoc.exists) throw { code: 'custom/userid-not-found' };
        await auth.signInWithEmailAndPassword(idxDoc.data().internal_email, password);
      }
    } catch (err) {
  ...
  function friendlyAuthError(err) {
    ...
    if (code === 'custom/email-not-allowed') return 'Akun tambahan tidak bisa masuk lewat Email — gunakan tab "User ID (Akun Tambahan)".';
    if (code === 'custom/userid-required') return 'Masukkan User ID.';
    if (code === 'custom/userid-not-found') return 'User ID tidak ditemukan.';
    if (code === 'auth/invalid-credential' || code === 'auth/wrong-password' || code === 'auth/user-not-found') {
      return loginMode === 'email' ? 'Email atau password salah.' : 'User ID atau password salah.';
    }
  ...
  function renderStats() {
    ...
    const osisCount = allRecords.filter((r) => r.role === 'Pengurus OSIS').length;
    const umumCount = allRecords.filter((r) => r.role === 'Peserta Umum').length;
    const panitiaCount = allRecords.filter((r) => r.role === 'Panitia').length;
    ...
    document.getElementById('statsGrid').innerHTML = `
      ...
      <div class="stat-card">
        <div class="stat-card__value">${umumCount}</div>
        <div class="stat-card__label">Peserta Umum</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${osisCount}</div>
        <div class="stat-card__label">Pengurus OSIS</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${panitiaCount}</div>
        <div class="stat-card__label">Panitia</div>
      </div>
    `;
  }
  ...
  [searchInput, roleFilter].forEach((el) => el.addEventListener('input', renderTable));
  ...
  function renderTable() {
    ...
    recordsBody.innerHTML = records.map((r) => {
      const suppliesBadge = r.supplies_done
        ? '<span class="badge badge--success">Lengkap</span>'
        : '<span class="badge badge--muted">Belum</span>';
      const submittedStr = r.submitted_at && r.submitted_at.toDate
        ? r.submitted_at.toDate().toLocaleDateString('id-ID')
        : '-';

      return `
        <tr class="row-clickable" data-row-id="${r.id}" tabindex="0" role="button" aria-label="Lihat detail ${escapeHtml(r.full_name)}">
          ...
          <td>
            <div class="row-actions">
              <button class="btn btn--primary btn--small" data-contact="${r.id}">Hubungi</button>
            </div>
          </td>
        </tr>
      `;
    ...
    // Baris (di luar tombol Hubungi) bisa diklik untuk membuka detail
    // lengkap — Unduh PDF & Hapus data tetap tersedia di dalam modal
    // detail tsb. Kolom "Aksi" sengaja hanya menampilkan "Hubungi" di
    // tampilan tabel.
    recordsBody.querySelectorAll('tr[data-row-id]').forEach((tr) => {
      const open = () => openDetail(tr.dataset.rowId);
      tr.addEventListener('click', (e) => {
        if (e.target.closest('[data-contact]')) return;
        open();
      });
      tr.addEventListener('keydown', (e) => {
        if ((e.key === 'Enter' || e.key === ' ') && !e.target.closest('[data-contact]')) {
          e.preventDefault();
          open();
        }
      });
    });
    recordsBody.querySelectorAll('[data-contact]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const r = allRecords.find((x) => x.id === btn.dataset.contact);
        if (r) openContactForRecord(r);
      });
    });
  }
  ...
  async function openDetail(id) {
    ...
    const { plan, computed: planComputed } = getRecordPlan(r);
    const planBadgeClass = plan.priority === 'Prioritas Tinggi' ? 'badge--alert'
      : (plan.priority === 'Perlu Perhatian' ? 'badge--warn' : 'badge--muted');
    ...
    modalBody.innerHTML = `
      ...
      <div class="detail-section">
        <h4>Rencana Penanganan Medis Otomatis <span class="badge ${planBadgeClass}">${plan.priority}</span></h4>
        <p class="contact-note">${escapeHtml(plan.intro)}${planComputed ? ' <em>(dihitung ulang otomatis — belum tersimpan karena data ini dikirim sebelum fitur ini ada)</em>' : ''}</p>
        ${plan.points.map((p) => `<p class="contact-note">• ${escapeHtml(p)}</p>`).join('')}
        <p class="contact-note">${escapeHtml(plan.priority_note)}</p>
      </div>
      <div class="detail-section">
        <h4>Kontak</h4>
      ...
      <div class="detail-section">
        ...
        <div id="wellbeingNoteBox">
          ${r.wants_to_explain === 'yes' ? '<p class="contact-note">Memuat cerita peserta...</p>' : ''}
        </div>
    ...
    // Isi "Ceritakan secara singkat" (Step 5) TIDAK ada di dokumen /participants
    // sama sekali — sengaja disimpan terpisah di /wellbeing_notes yang hanya
    // bisa dibaca admin utama atau akun admin yang staff_id-nya cocok dengan
    // guru/staf yang dipilih peserta ini (lihat firestore.rules). Jika akun
    // yang sedang login bukan salah satu dari itu, permintaan baca ini akan
    // ditolak Firestore — kondisi tsb ditangani dengan pesan yang jelas,
    // bukan dianggap error.
    if (r.wants_to_explain === 'yes') {
      const requestedId = r.id;
      try {
        const snap = await db.collection('wellbeing_notes').doc(r.id).get();
        if (!currentRecord || currentRecord.id !== requestedId) return; // modal sudah pindah ke record lain
        const box = document.getElementById('wellbeingNoteBox');
        if (!box) return;
        const text = snap.exists ? (snap.data().issue_detail || '') : '';
        box.innerHTML = text
          ? `<p class="contact-note">&ldquo;${escapeHtml(text)}&rdquo;</p>`
          : '<p class="contact-note">Peserta belum menuliskan cerita apa pun.</p>';
      } catch (err) {
        if (!currentRecord || currentRecord.id !== requestedId) return;
        const box = document.getElementById('wellbeingNoteBox');
        if (box) {
          const isPrimaryViewer = !!(auth.currentUser && auth.currentUser.email
            && auth.currentUser.email.toLowerCase() === PRIMARY_ADMIN_EMAIL);
          // Akun utama TIDAK PERNAH seharusnya ditolak membaca wellbeing_notes
          // (lihat firestore.rules) — kalau ini terjadi, hampir pasti karena
          // firestore.rules terbaru belum dipublikasikan ke Firebase Console,
          // bukan karena akun ini memang bukan guru yang dipilih. Untuk akun
          // lain, penolakan ini kemungkinan besar memang benar (mereka bukan
          // guru yang dipilih peserta ini), jadi pesannya tidak perlu
          // menyiratkan ada yang salah.
          const hint = isPrimaryViewer
            ? ' ⚠️ Akun utama seharusnya selalu bisa membaca ini — kemungkinan besar firestore.rules terbaru belum dipublikasikan ke Firebase Console (lihat README, "Langkah 1 — Menyiapkan Firebase", langkah 3 (tab Rules)).'
            : '';
          box.innerHTML = `<p class="contact-note">🔒 Cerita ini bersifat rahasia dan hanya dapat dilihat oleh ${escapeHtml(r.target_staff_name || 'guru/staf yang dipilih peserta')}.${hint}</p>`;
        }
      }
    }
  }
  ...
  async function deleteRecordById(id) {
    const r = allRecords.find((x) => x.id === id);
    const label = r ? `${r.full_name} (${r.class})` : 'peserta ini';
    if (!confirm(`Hapus data ${label} secara permanen? Tindakan ini tidak bisa dibatalkan.`)) return;
    try {
      // Dihapus dalam satu batch supaya /participants dan /wellbeing_notes
      // (cerita Langkah 5 peserta ini, kalau ada) selalu terhapus bersamaan
      // — "Hapus Data" tidak boleh meninggalkan cerita pribadi yatim piatu
      // tersimpan selamanya di Firestore setelah datanya "dihapus".
      const batch = db.batch();
      batch.delete(db.collection('participants').doc(id));
      batch.delete(db.collection('wellbeing_notes').doc(id));
      await batch.commit();
      if (currentRecord && currentRecord.id === id) { detailModal.hidden = true; currentRecord = null; }
  ...
  deleteBtn.addEventListener('click', () => {
    ...
    deleteRecordById(currentRecord.id);
  });
  ...
  async function fetchWellbeingText(p) {
    if (p.wants_to_explain !== 'yes') return null;
    try {
      const snap = await db.collection('wellbeing_notes').doc(p.id).get();
      const text = snap.exists ? (snap.data().issue_detail || '') : '';
      return text || 'Peserta belum menuliskan cerita apa pun.';
    } catch (err) {
      const isPrimaryViewer = !!(auth.currentUser && auth.currentUser.email
        && auth.currentUser.email.toLowerCase() === PRIMARY_ADMIN_EMAIL);
      if (isPrimaryViewer) {
        return `[Gagal dimuat — akun utama seharusnya selalu bisa membaca ini; firestore.rules terbaru kemungkinan belum dipublikasikan ke Firebase Console (lihat README, "Langkah 1 — Menyiapkan Firebase", langkah 3 (tab Rules))]`;
      }
      return `[Rahasia — hanya dapat dilihat oleh ${p.target_staff_name || 'guru/staf yang dipilih peserta'}]`;
    }
  }

  function renderParticipant(doc, p, wellbeingText) {
    ...
    {
      const { plan: pdfPlan, computed: pdfPlanComputed } = getRecordPlan(p);
      y = ensureSpace(doc, y, 60);
      y = sectionTitle(doc, y, `Rencana Penanganan Medis Otomatis — ${pdfPlan.priority}`);
      y = field(doc, y, 'Keterangan', pdfPlan.intro + (pdfPlanComputed ? ' (dihitung ulang otomatis, belum tersimpan)' : ''));
      (pdfPlan.points || []).forEach((pt) => {
        y = ensureSpace(doc, y, 24);
        y = field(doc, y, '•', pt, { alert: pdfPlan.priority === 'Prioritas Tinggi' });
      });
      if (pdfPlan.priority_note) {
        y = field(doc, y, 'Arti prioritas', pdfPlan.priority_note);
      }
    }
    ...
    if (p.has_personal_issue === 'yes') {
      ...
      if (p.wants_to_explain === 'yes') {
        y = field(doc, y, 'Penjelasan peserta', wellbeingText || '-');
  ...
  async function downloadPdfForRecord(r) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ unit: 'pt', format: 'a4' });
    const wellbeingText = await fetchWellbeingText(r);
    renderParticipant(doc, r, wellbeingText);
    doc.save(buildPdfFilename(r.full_name));
  }
  ...
  pdfBtn.addEventListener('click', async () => {
    ...
    pdfBtn.textContent = 'Menyiapkan...';
    try {
      await downloadPdfForRecord(currentRecord);
    } finally {
  ...
  document.getElementById('exportPdfBtn').addEventListener('click', async (e) => {
    ...
    try {
      ...
      if (!records.length) {
        ...
        // Diambil paralel supaya ekspor massal tidak lambat menunggu satu-satu.
        const wellbeingTexts = await Promise.all(records.map((p) => fetchWellbeingText(p)));
        records.forEach((p, idx) => {
          if (idx > 0) doc.addPage();
          renderParticipant(doc, p, wellbeingTexts[idx]);
        });
      }
  ...
  document.getElementById('exportCsvBtn').addEventListener('click', () => {
    ...
    records.forEach((r) => {
      const row = { ...r };
      const { plan: csvPlan } = getRecordPlan(r);
      row.medical_attention_priority = csvPlan.priority;
      row.medical_attention_points = (csvPlan.points || []).join(' | ');
  ...
  function openContactForRecord(r) {
  ...
  contactBtn.addEventListener('click', () => {
    if (!currentRecord) return;
    openContactForRecord(currentRecord);
  });
  ...
  // Dipakai oleh tab "Catatan Kesehatan Acara" (IIFE terpisah) untuk mengisi
  // saran nama peserta & auto-isi kelas, tanpa perlu langganan Firestore
  // kedua untuk data yang sama.
  window.getAdminRecords = () => allRecords;

  // ==========================================================================
  // TAB "ANALISIS MEDIS" — rekomendasi penanganan medis otomatis untuk
  // SELURUH peserta (Peserta Umum, Pengurus OSIS, Panitia) dalam satu
  // tampilan, diurutkan dari prioritas tertinggi. Rencana yang sudah
  // tersimpan (dibuat formulir saat pengiriman) dipakai apa adanya; data
  // lama yang belum memilikinya dihitung ulang di sini secara langsung
  // lewat mesin aturan bersama assets/js/medical-plan.js, supaya tidak ada
  // peserta yang "tidak kelihatan" di analisis ini hanya karena datanya
  // dikirim sebelum fitur ini ada.
  // ==========================================================================
  const analysisSearchInput = document.getElementById('analysisSearchInput');
  const analysisRoleFilter = document.getElementById('analysisRoleFilter');
  const analysisPriorityFilter = document.getElementById('analysisPriorityFilter');
  const analysisStatsGrid = document.getElementById('analysisStatsGrid');
  const analysisList = document.getElementById('analysisList');
  const analysisEmptyState = document.getElementById('analysisEmptyState');

  if (analysisSearchInput) {
    [analysisSearchInput, analysisRoleFilter, analysisPriorityFilter].forEach((el) => el.addEventListener('input', renderAnalysis));
  }

  function getRecordPlan(r) {
    if (r.medical_attention_plan) return { plan: r.medical_attention_plan, computed: false };
    return { plan: window.MedicalPlan.build(r), computed: true };
  }

  function renderAnalysis() {
    if (!analysisList) return; // panel tidak ada di halaman ini

    const withPlans = allRecords.map((r) => {
      const { plan, computed } = getRecordPlan(r);
      return { r, plan, computed };
    });

    const counts = { 'Prioritas Tinggi': 0, 'Perlu Perhatian': 0, Normal: 0 };
    withPlans.forEach(({ plan }) => { counts[plan.priority] = (counts[plan.priority] || 0) + 1; });
    analysisStatsGrid.innerHTML = `
      <div class="stat-card stat-card--alert">
        <div class="stat-card__value">${counts['Prioritas Tinggi']}</div>
        <div class="stat-card__label">Prioritas Tinggi</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${counts['Perlu Perhatian']}</div>
        <div class="stat-card__label">Perlu Perhatian</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${counts.Normal}</div>
        <div class="stat-card__label">Normal</div>
      </div>
      <div class="stat-card">
        <div class="stat-card__value">${withPlans.length}</div>
        <div class="stat-card__label">Total dianalisis</div>
      </div>
    `;

    const q = analysisSearchInput.value.trim().toLowerCase();
    const role = analysisRoleFilter.value;
    const priorityWanted = analysisPriorityFilter.value;
    const filtered = withPlans.filter(({ r, plan }) => {
      if (q && !`${r.full_name} ${r.class} ${r.major}`.toLowerCase().includes(q)) return false;
      if (role && r.role !== role) return false;
      if (priorityWanted && plan.priority !== priorityWanted) return false;
      return true;
    });

    filtered.sort((a, b) => (window.MedicalPlan.PRIORITY_RANK[b.plan.priority] || 0) - (window.MedicalPlan.PRIORITY_RANK[a.plan.priority] || 0)
    ...
    analysisList.innerHTML = filtered.map(({ r, plan, computed }) => {
      const badgeClass = plan.priority === 'Prioritas Tinggi' ? 'badge--alert' : (plan.priority === 'Perlu Perhatian' ? 'badge--warn' : 'badge--muted');
      return `
        <article class="analysis-card">
          <div class="analysis-card__head">
            <div>
              <h4>${escapeHtml(r.full_name)}</h4>
              <p class="analysis-card__meta">${escapeHtml(r.class)} · ${escapeHtml(r.major)} · ${escapeHtml(r.role)}</p>
            </div>
            <span class="badge ${badgeClass}">${plan.priority}</span>
          </div>
          <p class="contact-note">${escapeHtml(plan.intro)}${computed ? ' <em>(dihitung ulang otomatis — belum tersimpan karena data ini dikirim sebelum fitur ini ada)</em>' : ''}</p>
          <ul class="analysis-card__points">${plan.points.map((p) => `<li>${escapeHtml(p)}</li>`).join('')}</ul>
          <p class="contact-note">${escapeHtml(plan.priority_note)}</p>
          <div class="row-actions">
            <button class="btn btn--ghost btn--small" data-detail="${r.id}">Lihat Detail</button>
            <button class="btn btn--ghost btn--small" data-pdf="${r.id}">Unduh PDF</button>
            <button class="btn btn--ghost btn--small" data-contact="${r.id}">Hubungi</button>
            <button class="btn btn--ghost btn--small danger-ghost" data-delete="${r.id}">Hapus Data</button>
          </div>
        </article>
      `;
    }).join('');

    analysisList.querySelectorAll('[data-detail]').forEach((btn) => {
      btn.addEventListener('click', () => openDetail(btn.dataset.detail));
    });
    analysisList.querySelectorAll('[data-pdf]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        const r = allRecords.find((x) => x.id === btn.dataset.pdf);
        if (!r) return;
        const originalLabel = btn.textContent;
        btn.disabled = true;
        btn.textContent = 'Menyiapkan...';
        try {
          await downloadPdfForRecord(r);
        } finally {
          btn.disabled = false;
          btn.textContent = originalLabel;
        }
    ...
    analysisList.querySelectorAll('[data-contact]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const r = allRecords.find((x) => x.id === btn.dataset.contact);
        if (r) openContactForRecord(r);
      });
    });
    analysisList.querySelectorAll('[data-delete]').forEach((btn) => {
      btn.addEventListener('click', () => deleteRecordById(btn.dataset.delete));
    });
  }
})();

// ============================================================================
// TOGGLE TAMPILKAN/SEMBUNYIKAN PASSWORD — berlaku di seluruh Panel Admin:
// form masuk (login) maupun tab "Akun Admin" (Akun Saya & Tambah Admin Baru).
// ============================================================================
(function wirePasswordToggles() {
  document.querySelectorAll('.password-toggle').forEach((btn) => {
    ...
    btn.addEventListener('click', () => {
      ...
      btn.title = nowShowing ? 'Sembunyikan password' : 'Tampilkan password';
      if (window.SoundFX) window.SoundFX.click();
    });
  });
})();
...
// "Akun Admin"). Akun "utama/bawaan sistem" (dibuat langsung lewat Firebase
...
(function () {
  ...
  let accountRows = [];
  let staffOptionsForAccounts = [];
  let staffOptionsUnsub = null;

  function renderStaffIdOptions(selectEl, selectedId) {
    const options = ['<option value="">Tidak ditautkan ke guru/staf tertentu</option>']
      .concat(staffOptionsForAccounts.map((s) => `<option value="${s.id}" ${s.id === selectedId ? 'selected' : ''}>${escapeHtml(s.name)}</option>`));
  ...
  function friendlyAccountAuthError(err) {
    ...
    if (code === 'auth/email-already-in-use') return 'User ID ini sudah dipakai — coba User ID lain.';
    if (code === 'auth/invalid-email') return 'User ID mengandung karakter yang tidak didukung.';
  ...
  auth.onAuthStateChanged(async (user) => {
    if (!user) {
      ...
      if (staffOptionsUnsub) { staffOptionsUnsub(); staffOptionsUnsub = null; }
      accountsBody.innerHTML = '';
      return;
    }

    if (staffOptionsUnsub) staffOptionsUnsub();
    staffOptionsUnsub = db.collection('staff').orderBy('name').onSnapshot((snap) => {
      staffOptionsForAccounts = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
      const newAdminSelect = document.getElementById('newAdminStaffId');
      if (newAdminSelect) renderStaffIdOptions(newAdminSelect, '');
      renderAccountsTable();
    }, () => { staffOptionsForAccounts = []; });
    ...
    try {
      const doc = await db.collection('admin_accounts').doc(user.uid).get();
      selfDoc = doc.exists ? doc.data() : null;
    } catch (err) {
      selfDoc = null; // gagal periksa -> aman: perlakukan sebagai akun utama (sembunyikan)
    }

    // Akun utama (dikenali lewat email persisnya) TIDAK PERNAH boleh
    // menampilkan fitur "Ubah Password" di sini, apapun status dokumen
    // admin_accounts-nya — pengaman tambahan selain pengecekan selfDoc di atas.
    const isPrimaryAccount = !!(user.email && user.email.toLowerCase() === PRIMARY_ADMIN_EMAIL);

    if (selfDoc && !isPrimaryAccount) {
      selfDesc.textContent = `Kamu masuk sebagai User ID "${selfDoc.user_id}"${selfDoc.label ? ` (${selfDoc.label})` : ''}. Kamu bisa mengubah password akun ini sendiri di bawah (perlu memasukkan password saat ini).`;
      selfForm.hidden = false;
    } else {
      selfDesc.textContent = 'Akun ini adalah akun utama/bawaan sistem (dibuat langsung lewat Firebase Console saat pengaturan awal, dan satu-satunya yang boleh masuk lewat Email). Demi keamanan, email dan password akun ini sengaja disembunyikan & tidak bisa diubah, dihapus, atau diedit lewat portal ini — hubungi pemegang akses Firebase Console bila perlu menggantinya.';
      selfForm.hidden = true;
    }

    // ---------- Daftar admin tambahan yang dibuat lewat portal ini ----------
    if (accountsUnsub) accountsUnsub();
    accountsUnsub = db.collection('admin_accounts').orderBy('created_at', 'desc').onSnapshot((snap) => {
      accountRows = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
      renderAccountsTable();
  ...
  function renderAccountsTable() {
    ...
    accountsBody.innerHTML = accountRows.map((r) => `
      <tr>
        <td>${escapeHtml(r.label || '-')}</td>
        <td>${escapeHtml(r.user_id)}</td>
        <td>${r.created_at && r.created_at.toDate ? r.created_at.toDate().toLocaleString('id-ID') : '-'}</td>
        <td>
          <select class="staff-link-select" data-account-uid="${escapeHtml(r.id)}" style="min-height:34px; font-size:13px;"></select>
        </td>
        <td><button type="button" class="link-btn link-btn--danger" data-delete-uid="${escapeHtml(r.id)}" data-delete-userid="${escapeHtml(r.user_id)}">Hapus</button></td>
      </tr>
    `).join('');
    accountsBody.querySelectorAll('.staff-link-select').forEach((sel) => {
      const row = accountRows.find((r) => r.id === sel.dataset.accountUid);
      renderStaffIdOptions(sel, row ? (row.staff_id || '') : '');
      sel.addEventListener('change', async () => {
        const uid = sel.dataset.accountUid;
        const chosen = staffOptionsForAccounts.find((s) => s.id === sel.value);
        sel.disabled = true;
        try {
          await db.collection('admin_accounts').doc(uid).update({
            staff_id: sel.value || null,
            staff_name: chosen ? chosen.name : null,
          });
          if (window.SoundFX) window.SoundFX.click();
        } catch (err) {
          window.alert(err && err.message ? err.message : 'Gagal menautkan akun ke guru/staf. Silakan coba lagi.');
        } finally {
          sel.disabled = false;
        }
      });
  ...
  // ---------- Hapus akun tambahan (menonaktifkan login User ID-nya) ----------
  accountsBody.addEventListener('click', async (e) => {
    ...
    if (!window.confirm(`Hapus akun admin dengan User ID "${userId}"? Akun ini tidak akan bisa masuk lagi lewat User ID setelah dihapus.`)) return;
    btn.disabled = true;
    btn.textContent = 'Menghapus...';
    try {
      await db.collection('admin_login_index').doc(userId).delete();
      await db.collection('admin_accounts').doc(uid).delete();
      if (window.SoundFX) window.SoundFX.click();
    } catch (err) {
      window.alert(err && err.message ? err.message : 'Gagal menghapus akun. Silakan coba lagi.');
      btn.disabled = false;
  ...
  // ---------- Tambah admin baru (User ID + password) ----------
  addForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    addError.hidden = true;
    addSuccess.hidden = true;
    const label = document.getElementById('newAdminLabel').value.trim();
    const userId = sanitizeUserId(document.getElementById('newAdminUserId').value);
    const password = document.getElementById('newAdminPassword').value;
    if (!isValidUserId(userId)) {
      addError.textContent = 'User ID harus 3-32 karakter: huruf, angka, titik, garis bawah, atau strip (tanpa spasi/simbol @).';
    ...
    try {
      const existingIndex = await db.collection('admin_login_index').doc(userId).get();
      ...
      const chosenStaff = staffOptionsForAccounts.find((s) => s.id === document.getElementById('newAdminStaffId').value);
      await db.collection('admin_accounts').doc(newUid).set({
        user_id: userId,
        label: label || '',
        staff_id: chosenStaff ? chosenStaff.id : null,
        staff_name: chosenStaff ? chosenStaff.name : null,
        created_by: auth.currentUser ? auth.currentUser.uid : null,
        created_at: firebase.firestore.FieldValue.serverTimestamp(),
      });
      await db.collection('admin_login_index').doc(userId).set({
        uid: newUid,
        internal_email: internalEmail,
      });

      addSuccess.textContent = `Akun admin dengan User ID "${userId}" berhasil ditambahkan. Bagikan User ID & password ini secara aman kepada yang bersangkutan — mereka masuk lewat tab "User ID (Akun Tambahan)".`;
...
(function () {
  ...
  function staffOrRecordRow(id) {
    const row = document.querySelector(`#recordsBody tr[data-row-id="${id}"]`);
    return row || null;
  }
})();

// ============================================================================
// TAB "CATATAN KESEHATAN ACARA" — log kejadian kesehatan yang dicatat
// LANGSUNG oleh guru/staf medis SELAMA acara berlangsung (berbeda dari
// /participants, yang diisi peserta sendiri SEBELUM acara). Dipakai untuk
// menyusun laporan medis kegiatan (PDF/CSV). Koleksi Firestore terpisah:
// event_health_logs — sepenuhnya privat, hanya admin yang login yang bisa
// membaca/menulis (lihat firestore.rules).
// ============================================================================
(function () {
  const form = document.getElementById('eventLogForm');
  if (!form) return; // aman bila markup tab ini tidak ada di halaman

  const nameInput = document.getElementById('logParticipantName');
  const classInput = document.getElementById('logParticipantClass');
  const dateInput = document.getElementById('logDate');
  const timeInput = document.getElementById('logTime');
  const locationInput = document.getElementById('logLocation');
  const conditionInput = document.getElementById('logCondition');
  const actionInput = document.getElementById('logAction');
  const severityInput = document.getElementById('logSeverity');
  const followUpInput = document.getElementById('logFollowUp');
  const submitBtn = document.getElementById('eventLogSubmitBtn');
  const formError = document.getElementById('eventLogFormError');
  const datalist = document.getElementById('logParticipantOptions');
  const searchInput = document.getElementById('eventLogSearchInput');
  const severityFilter = document.getElementById('eventLogSeverityFilter');
  const todayOnlyInput = document.getElementById('eventLogTodayOnly');
  const tbody = document.getElementById('eventLogBody');
  const emptyState = document.getElementById('eventLogEmptyState');
  const exportCsvBtn = document.getElementById('eventLogExportCsvBtn');
  const exportPdfBtn = document.getElementById('eventLogExportPdfBtn');
  ...
  function setDefaultDateTime() {
    ...
    if (!timeInput.value) timeInput.value = nowTimeStr();
  }
  setDefaultDateTime();

  // ---------- Saran nama peserta (dari data yang sudah dimuat di tab Dashboard) ----------
  function refreshParticipantDatalist() {
    const records = window.getAdminRecords ? window.getAdminRecords() : [];
    datalist.innerHTML = records.map((r) => `<option value="${escapeHtml(r.full_name)}">${escapeHtml(r.class)}</option>`).join('');
  }
  nameInput.addEventListener('focus', refreshParticipantDatalist);

  let matchedParticipant = null;
  nameInput.addEventListener('input', () => {
    const records = window.getAdminRecords ? window.getAdminRecords() : [];
    const typed = nameInput.value.trim().toLowerCase();
    const match = records.find((r) => (r.full_name || '').trim().toLowerCase() === typed);
    matchedParticipant = match || null;
    if (match) classInput.value = match.class || '';
  });
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!participantName) { showFormError('Nama peserta wajib diisi.'); return; }
    if (!condition) { showFormError('Kondisi/keluhan yang diamati wajib diisi.'); return; }
    if (!actionTaken) { showFormError('Tindakan yang diberikan wajib diisi.'); return; }
    if (!dateInput.value || !timeInput.value) { showFormError('Tanggal & waktu kejadian wajib diisi.'); return; }

    const occurredAt = window.SupplyPhase.parseLocal(dateInput.value, timeInput.value);
    if (!occurredAt) { showFormError('Tanggal/waktu kejadian tidak valid.'); return; }

    const adminNameEl = document.getElementById('adminName');
    const recordedBy = (adminNameEl && adminNameEl.textContent) || (auth.currentUser ? auth.currentUser.email : '') || '-';

    submitBtn.disabled = true;
    submitBtn.textContent = 'Menyimpan...';
    try {
      await db.collection('event_health_logs').add({
        participant_name: participantName,
        participant_class: classInput.value.trim() || (matchedParticipant ? matchedParticipant.class || '' : ''),
        participant_id: matchedParticipant ? matchedParticipant.id : null,
        location: locationInput.value.trim(),
        condition,
        action_taken: actionTaken,
        severity: severityInput.value,
        follow_up_needed: !!followUpInput.checked,
        occurred_at: firebase.firestore.Timestamp.fromDate(occurredAt),
        recorded_by: recordedBy,
        recorded_by_uid: auth.currentUser ? auth.currentUser.uid : null,
        created_at: firebase.firestore.FieldValue.serverTimestamp(),
      });
      form.reset();
      ...
      showFormError('Gagal menyimpan catatan: ' + (err && err.message ? err.message : err));
    } finally {
  ...
  function getFilteredLogs() {
    ...
    const todayOnly = todayOnlyInput.checked;
    return logRows.filter((r) => {
      if (q && !(r.participant_name || '').toLowerCase().includes(q)) return false;
      if (severity && r.severity !== severity) return false;
      if (todayOnly && !isToday(r.occurred_at)) return false;
      return true;
    });
  }

  function severityBadgeClass(sev) {
    if (sev === 'Berat') return 'badge--alert';
    if (sev === 'Sedang') return 'badge--warn';
    return 'badge--muted';
  }
  ...
  function renderLogTable() {
    ...
    tbody.innerHTML = rows.map((r) => {
      const occurredStr = r.occurred_at && r.occurred_at.toDate ? r.occurred_at.toDate().toLocaleString('id-ID') : '-';
      return `
        <tr>
          <td>${occurredStr}</td>
          <td>${escapeHtml(r.participant_name)}</td>
          <td>${escapeHtml(r.participant_class)}</td>
          ...
          <td>
            ...
            ${r.follow_up_needed ? ' <span class="badge badge--warn">Tindak lanjut</span>' : ''}
          </td>
          <td>${escapeHtml(r.recorded_by)}</td>
          <td><button class="link-btn link-btn--danger" data-del="${r.id}">Hapus</button></td>
        </tr>
      `;
    }).join('');
    ...
    tbody.querySelectorAll('[data-del]').forEach((btn) => {
      btn.addEventListener('click', () => {
        ...
        db.collection('event_health_logs').doc(btn.dataset.del).delete()
          .catch((err) => showFormError('Gagal menghapus catatan: ' + (err && err.message ? err.message : err)));
      });
    });
  }

  [searchInput, severityFilter, todayOnlyInput].forEach((el) => el.addEventListener('input', renderLogTable));

  let logUnsub = null;
  auth.onAuthStateChanged((user) => {
    if (user) {
      logUnsub = db.collection('event_health_logs').orderBy('occurred_at', 'desc').onSnapshot((snap) => {
        logRows = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        renderLogTable();
      }, (err) => {
        showFormError('Gagal memuat catatan kesehatan: ' + (err && err.message ? err.message : err));
      });
    } else if (logUnsub) {
      logUnsub(); logUnsub = null;
      logRows = [];
    }
  });
  ...
  exportPdfBtn.addEventListener('click', () => {
    const rows = getFilteredLogs();
    if (!rows.length) { alert('Tidak ada catatan untuk diekspor (periksa filter yang aktif).'); return; }
    ...
    doc.text('Laporan Kesehatan Acara', marginX, y);
    y += 20;
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.text(`Dicetak: ${new Date().toLocaleString('id-ID')} — Total catatan: ${rows.length}`, marginX, y);
    y += 24;
    ...
    rows.forEach((r, idx) => {
      ...
      doc.setFontSize(11);
      doc.setFont(undefined, 'bold');
      doc.text(`${idx + 1}. ${r.participant_name || '-'} (${r.participant_class || '-'}) — ${r.severity || '-'}`, marginX, y);
      y += 15;

      doc.setFontSize(9.5);
      doc.setFont(undefined, 'normal');
      const metaLine = `Waktu: ${occurredStr}${r.location ? ' · Lokasi: ' + r.location : ''}`;
      let lines = doc.splitTextToSize(metaLine, 500);
      doc.text(lines, marginX + 8, y); y += lines.length * 12 + 3;

      lines = doc.splitTextToSize(`Kondisi: ${r.condition || '-'}`, 500);
      doc.text(lines, marginX + 8, y); y += lines.length * 12 + 3;

      lines = doc.splitTextToSize(`Tindakan: ${r.action_taken || '-'}`, 500);
      doc.text(lines, marginX + 8, y); y += lines.length * 12 + 3;

      if (r.follow_up_needed) {
        doc.setFont(undefined, 'bold');
        doc.text('Perlu tindak lanjut lebih lanjut.', marginX + 8, y);
        doc.setFont(undefined, 'normal');
        y += 13;
      }
      doc.text(`Dicatat oleh: ${r.recorded_by || '-'}`, marginX + 8, y);
      y += 22;
    });

    doc.save(`laporan-kesehatan-acara-${todayStr()}.pdf`);
  });
  ...
  exportCsvBtn.addEventListener('click', () => {
    ...
    const headers = ['Waktu kejadian', 'Nama', 'Kelas', 'Lokasi', 'Kondisi', 'Tindakan', 'Tingkat', 'Perlu tindak lanjut', 'Dicatat oleh'];
    const escape = (v) => `"${String(v === undefined || v === null ? '' : v).replace(/"/g, '""')}"`;
    const lines = [headers.join(',')];
    rows.forEach((r) => {
      const occurredStr = r.occurred_at && r.occurred_at.toDate ? r.occurred_at.toDate().toLocaleString('id-ID') : '';
      lines.push([
        occurredStr, r.participant_name, r.participant_class, r.location,
        r.condition, r.action_taken, r.severity, r.follow_up_needed ? 'Ya' : 'Tidak', r.recorded_by,
      ].map(escape).join(','));
    });
    const blob = new Blob(['\uFEFF' + lines.join('\n')], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `catatan-kesehatan-acara-${todayStr()}.csv`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
