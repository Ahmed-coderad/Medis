(function () {
  ...
  function openDetail(id) {
    ...
    modalBody.innerHTML = `
      ...
      <div class="detail-section">
        <h4>Obat-obatan Pribadi yang Dibawa</h4>
        ${meds.length ? meds.map((m) => detailRow(m.name, `${m.dosage || '-'} · ${m.schedule || '-'}`)).join('') : '<p class="contact-note">Tidak membawa obat.</p>'}
        ${meds.length && r.medications_schedule ? detailRow('Jadwal (sesuai Jadwal Perlengkapan admin)', r.medications_schedule) : ''}
      </div>
      <div class="detail-section">
        <h4>Perlengkapan Ibadah &amp; Mandi</h4>
  ...
  function renderParticipant(doc, p) {
    ...
    y = sectionTitle(doc, y, 'Obat-obatan Pribadi yang Dibawa');
    ...
    if (!meds.length) {
      ...
      if (p.medications_schedule) y = field(doc, y, 'Jadwal (sesuai jadwal admin)', p.medications_schedule);
    }
