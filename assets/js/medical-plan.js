...
(function (global) {
  ...
  function build(d) {
    ...
    if (d.has_personal_issue === 'yes') {
      // Catatan: field issue_detail (isi ceritanya) SENGAJA tidak pernah ada
      // di sini — disimpan terpisah di /wellbeing_notes demi privasi (hanya
      // bisa dibaca guru/staf yang dipilih peserta). Prioritas cukup
      // ditentukan dari has_personal_issue + wants_to_explain saja (dua
      // flag ini tetap tersimpan di /participants dan aman dibaca semua
      // admin), TANPA perlu tahu isi ceritanya.
      if (d.wants_to_explain === 'yes') {
