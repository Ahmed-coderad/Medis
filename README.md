...
## 🆕 Dashboard dirapikan & Laporan Medis Acara (terbaru)

- **Tab "Dashboard" disederhanakan supaya tidak tumpang tindih dengan tab
  "Analisis Medis"**. Dashboard sekarang murni untuk data registrasi/
  administratif: kartu statistik menampilkan Total peserta & jumlah per
  peran (Pengurus OSIS / Peserta Umum / Panitia), filter toolbar diganti
  jadi status perlengkapan (Sudah/Belum lengkap), dan kolom tabel jadi
  Nama, Kelas, Jurusan, Peran, Status Perlengkapan, Aksi — kolom Alergi &
  Masalah Pribadi dihapus dari sini karena sudah jadi fokus tab "Analisis
  Medis" sepenuhnya (satu-satunya tempat untuk info alergi, kondisi medis,
  dan kesejahteraan peserta).
- **Perbaikan kecil**: status "Belum lengkap perlengkapan" sekarang benar-
  benar ikut ter-update jadi "Sudah lengkap" begitu peserta menyusulkan
  obat/perlengkapan lewat pengingat — sebelumnya status ini tidak pernah
  berubah di data tersimpan walau peserta sudah melengkapinya.
- **Tab baru "Laporan Medis Acara"** di Portal Guru / Staf Medis — untuk
  mencatat kondisi kesehatan yang muncul **selama acara berlangsung** (mis.
  pusing, jatuh, demam mendadak), boleh ditautkan ke peserta terdaftar atau
  dicatat manual (mis. untuk guru pendamping/tamu). Setiap catatan berisi
  kondisi/gejala, tindakan yang diberikan, tingkat keparahan (Ringan/
  Sedang/Berat-Darurat), dan siapa yang mencatat — lalu bisa diunduh
  sebagai laporan (CSV atau PDF ringkasan) untuk keperluan pelaporan medis
  setelah kegiatan selesai. Terpisah dari data kesehatan yang peserta isi
  sendiri sebelum acara (tab Analisis Medis).

## Analisis Medis & aksi cepat di setiap baris data

- **Tab "Analisis Medis"** di panel admin: menampilkan rencana
...
- `medical_logs/{id}` — catatan kondisi kesehatan yang dicatat guru/staf
  medis **selama acara berlangsung** lewat tab "Laporan Medis Acara":
  `participant_id` (opsional, bila ditautkan ke peserta terdaftar),
  `person_name`, `person_class`, `severity` (`Ringan`/`Sedang`/
  `Berat/Darurat`), `condition`, `action_taken`, `recorded_by`,
  `recorded_at`. Terpisah dari `participants` supaya tidak tercampur
  dengan data yang peserta isi sendiri sebelum acara. Hanya admin yang
  login yang bisa membaca/menulis/menghapus.
...
assets/js/admin.js                       Logika dashboard, analisis medis, PDF, CSV, kontak, kelola staf, jadwal
