...
- `admin_accounts/{uid}` — metadata (email, label, pembuat, waktu dibuat)
  untuk admin yang ditambahkan lewat tab **Akun Admin** di panel admin.
  Kata sandi sesungguhnya tetap dikelola penuh oleh Firebase Authentication
  (tidak pernah disimpan di Firestore). Akun yang tidak punya dokumen di
  sini otomatis dianggap "akun bawaan sistem" (dibuat langsung lewat
  Firebase Console) dan disembunyikan/tidak bisa diubah lewat tab ini.
